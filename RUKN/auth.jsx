...
function LoginForm({ t, onSubmit }) {
  ...
  // Login accepts either email or username — only check non-empty here.
  const identifierValid = email.trim().length > 0;
  const pwValid = pw.length >= 1;

  const emailErr = touched.email && !identifierValid ? t.err_email : null;
  const pwErr = touched.pw && !pwValid ? t.err_password : null;
  const formValid = identifierValid && pwValid;

  // Login accepts email OR username, so relax the input type but keep the regex hint.
  return (
    <form className="auth-form" onSubmit={(e) => {
      e.preventDefault();
      if (!formValid || !onSubmit) return;
      onSubmit({ identifier: email, password: pw });
    }}>
      <Field label={t.emailLabel} type="text"
      ...
      <div className="form-meta">
        <a href="Forgot.html" className="link-quiet">
          {t.forgot}
        </a>
      </div>
...
function RegisterForm({ t, onSubmit }) {
  ...
  return (
    <form className="auth-form" onSubmit={(e) => {
      e.preventDefault();
      if (!formValid || !onSubmit) return;
      onSubmit({ name, email, password: pw, confirm, lang });
    }}>
...
function AuthScreen() {
  ...
  React.useEffect(() => {
    ...
    // Skip the login screen if a session is already alive.
    if (window.RuknAPI) {
      window.RuknAPI.me().then(r => {
        if (r.status === 200 && r.data.ok) window.location.href = "Dashboard.html";
      });
    }
  }, []);

  // Derive a backend-safe username from the free-form name field.
  // Backend regex: ^[a-z0-9_]{3,20}$
  const slugifyUsername = (raw) => {
    let s = (raw || "").toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
    if (s.length < 3) s = (s + "_user").slice(0, 20);
    return s.slice(0, 20);
  };

  const onSubmit = async (payload) => {
    if (!window.RuknAPI) {
      setServerError("API client missing. Reload the page.");
      return;
    }
    setServerError("");
    setBusy(true);
    try {
      if (tab === "login") {
        const r = await window.RuknAPI.login(payload.identifier, payload.password);
        if (r.data.ok) {
          window.location.href = "Dashboard.html";
        } else {
          setServerError(r.data.error || "Login failed.");
        }
      } else {
        const username = slugifyUsername(payload.name);
        const lang = (payload.lang || "en").toUpperCase() === "AR" ? "AR" : "EN";
        const r = await window.RuknAPI.register({
          username,
          email: payload.email,
        ...
        if (r.data.ok) {
          ...
          setServerError(r.data.error || "Registration failed.");
        }
      }
    } catch (err) {
      setServerError(String(err && err.message || err));
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="auth-stage">
      ...
      <main className="auth-right">
        ...
        <div className="auth-card">
          ...
          {serverError ? (
            <div className="field-error" style={{marginBottom: 14, padding: "10px 12px",
...
// Only auto-mount the login/register screen on Login.html itself. Forgot.html
// and Reset.html reuse the AuthVignette component but render their own apps.
if ((document.getElementById("root")?.getAttribute("data-screen-label") || "")
      .startsWith("02 Login")) {
