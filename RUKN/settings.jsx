  ...
  return (
    <section className="settings-card" aria-labelledby="danger-heading">
      ...
      <div className="danger-row">
        <div>
          <h3 className="settings-subhead no-mt">Sign out</h3>
          <p className="settings-hint">
            End this session on this device. Your books and notes will be waiting next time.
          </p>
        </div>
        <button className="btn btn-ghost"
                onClick={() => window.RuknAPI && window.RuknAPI.logout()}>
          Log out
        </button>
      </div>
...
function SettingsApp() {
  ...
  // Load real user + preferences from the backend on mount.
  React.useEffect(() => {
    ...
    (async () => {
      ...
      if (r.data.user) {
        setUser(u => ({
          ...u,
          username: r.data.user.username || u.username,
          email:    r.data.user.email    || u.email,
          preferred_lang: r.data.user.preferred_lang || u.preferred_lang,
          tier:     r.data.user.tier     || u.tier,
          display_name: r.data.user.username || u.display_name,
        }));
      }
      if (r.data.preferences) {
        setPrefs(p => ({
          ...p,
          theme:         r.data.preferences.theme         || p.theme,
          font:          r.data.preferences.font          || p.font,
          font_size:     r.data.preferences.font_size     || p.font_size,
          ambient_sound: r.data.preferences.ambient_sound || p.ambient_sound,
          sound_volume:  parseFloat(r.data.preferences.sound_volume) || p.sound_volume,
        }));
      }
    })();
  }, []);
  ...
  const onSavePrefs = async () => {
    if (window.RuknAPI) {
      const r = await window.RuknAPI.call("update_preference", {
        theme:         prefs.theme,
      ...
      if (!r.data.ok) { flashToast(r.data.error || "Save failed"); return; }
    }
  ...
  const onUpgrade = async () => {
    if (window.RuknAPI) {
      const r = await window.RuknAPI.call("upgrade");
      if (!r.data.ok) { flashToast(r.data.error || "Upgrade failed"); return; }
    }
  ...
  const confirmCancelSub = async () => {
    if (window.RuknAPI) {
      const r = await window.RuknAPI.call("downgrade");
      if (!r.data.ok) { flashToast(r.data.error || "Cancel failed"); return; }
    }
  ...
  const onExport = async () => {
    let bundle = { user, prefs };
    if (window.RuknAPI) {
      const r = await window.RuknAPI.call("export");
      if (r.data.ok) bundle = r.data.export;
    }
  ...
  const onDeleteConfirm = async () => {
    if (window.RuknAPI) {
      const r = await window.RuknAPI.call("delete_account");
      if (!r.data.ok) { flashToast(r.data.error || "Delete failed"); return; }
    }
    setDeleteOpen(false);
