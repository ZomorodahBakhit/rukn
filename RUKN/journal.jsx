...
function JournalApp() {
  ...
  // Map a backend row → the shape the existing entry-card/detail components expect.
  const mapEntry = (e) => {
    const created = e.created_at ? new Date(e.created_at) : new Date();
    return {
      id:    "e_" + e.entry_id,
      _real: true,                              // marker so we know it's from DB
      _dbId: e.entry_id,
      date:  created.toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" }),
      mood:  e.mood || "🌙",
      title: (e.content || "").split(/\s+/).slice(0, 4).join(" ") || "Entry",
      body:  e.content || "",
      tags:  Array.isArray(e.tags) ? e.tags : [],
      book:  null,
      rtl:   /[؀-ۿ]/.test(e.content || ""),  // Arabic char detection
    };
  };

  // Load real entries from the backend on mount.
  React.useEffect(() => {
    if (!window.RuknAPI) return;
    (async () => {
      const r = await window.RuknAPI.call("journal_list");
      if (r.data.ok && Array.isArray(r.data.entries)) {
        const mapped = r.data.entries.map(mapEntry);
        setEntries(mapped.length ? mapped : ENTRIES);
        if (mapped.length) setSelId(mapped[0].id);
      }
    })();
  }, []);
  ...
  const onSaveEntry = async (data) => {
    if (editing) {
      // No "update entry" endpoint in the current backend — we apply the edit locally only.
      // (A future iteration would add /api?action=update_entry and reuse this branch.)
      ...
      setEditing(null);
      setNewOpen(false);
      flashToast("Entry updated (local only)");
      return;
    }

    // New entry → persist via backend, then prepend the returned row.
    if (window.RuknAPI) {
      ...
      if (!r.data.ok) {
        flashToast(r.data.error || "Save failed");
        return;
      }
      const fresh = mapEntry(r.data.entry);
    ...
    // Offline fallback (no API loaded).
    const newEntry = {
      id: "e" + Date.now(),
      date: "Today",
      mood: data.mood,
  ...
  const onDelete = async () => {
    if (!selected) return;
    // Only DB-backed entries can be deleted through the API; mocks just disappear locally.
    if (selected._real && window.RuknAPI) {
      const r = await window.RuknAPI.call("delete_entry", { entry_id: selected._dbId });
      if (!r.data.ok) {
        flashToast(r.data.error || "Could not delete");
        return;
      }
    }
