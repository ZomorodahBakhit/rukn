...
function DashboardApp() {
  ...
  // Live dashboard data: continue-reading row, recent entries, totals.
  // continueBook === undefined → still loading; null → no active book; obj → real.
  const [continueBook, setContinueBook] = React.useState(undefined);
  const [stats, setStats]               = React.useState(null);
  const [user, setUser]                 = React.useState(null);

  React.useEffect(() => {
    if (!window.RuknAPI) { setContinueBook(null); return; }
    (async () => {
      const r = await window.RuknAPI.call("dashboard");
      if (r.data.ok) {
        setContinueBook(r.data.continueReading || null);
  ...
  const goReader  = () => {
    const id = continueBook && continueBook.book_id;
        ...
        <div className="windows">
          {open.has("continue") &&
            <DashWindow id="continue" layout={t.layout} onClose={() => toggle("continue")}>
              {continueBook === undefined
                ...
                : <ContinueReadingWidget t={tStr} onResume={goReader} data={continueBook}/>}
            </DashWindow>}
          {open.has("journal") &&
          ...
          {open.has("stats") &&
            <DashWindow id="stats" layout={t.layout} onClose={() => toggle("stats")}>
              <ReadingStatsWidget t={tStr} stats={stats}/>
            </DashWindow>}
          {open.has("sounds") &&
