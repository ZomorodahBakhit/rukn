...
function ContinueReadingWidget({ t, onResume }) {
  return (
    <>
      <div className="w-eyebrow">{t.continueReading}</div>
      <div className="cr-row">
        <div className="cr-cover">
          {/* Cozy book cover: warm earth tones */}
          <svg viewBox="0 0 80 110" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
            <rect width="80" height="110" fill="#8C3F23"/>
            <rect x="6" y="10" width="68" height="90" fill="none" stroke="#F2C77A" strokeWidth="0.8" opacity="0.6"/>
            <text x="40" y="46" textAnchor="middle" fill="#F2C77A"
                  fontFamily="Cormorant Garamond, serif" fontSize="11" fontStyle="italic"
                  letterSpacing="0.5">The</text>
            <text x="40" y="62" textAnchor="middle" fill="#F4E9D4"
                  fontFamily="Cormorant Garamond, serif" fontSize="14" fontWeight="500"
                  letterSpacing="0.5">Prophet</text>
            <line x1="20" y1="72" x2="60" y2="72" stroke="#F2C77A" strokeWidth="0.6" opacity="0.7"/>
            <text x="40" y="84" textAnchor="middle" fill="#F2C77A"
                  fontFamily="Manrope, sans-serif" fontSize="5" letterSpacing="1.5">GIBRAN</text>
          </svg>
        </div>
        <div className="cr-meta">
          <div className="cr-title display">{t.bookTitle}</div>
          <div className="cr-author">{t.bookAuthor}</div>
          <div className="cr-chapter">{t.chapter}</div>
          <div className="cr-progress">
            <div className="cr-progress-bar">
              <div className="cr-progress-fill" style={{ width: `${(47/96)*100}%` }}/>
            </div>
            <div className="cr-progress-text">{t.pages}</div>
          </div>
        </div>
      </div>
      ...
      <div className="stats-bar">
        {[3, 5, 2, 6, 4, 7, 3].map((v, i) => (
          <div key={i} className="stats-bar-day">
            <div className="stats-bar-fill" style={{ height: `${v * 12}%` }}/>
...
// ── Dashboard Top Bar ────────────────────────────────────────────────────────
function DashboardTopBar({ t, scene, setScene, onRearrange, onPremium, premium = false }) {
  const scenes = [
  ...
  return (
    <header className="dash-topbar">
      <div className="dash-greeting">
        ...
        <span className="dash-greet-name display">{t.userFirst}.</span>
      </div>
      ...
      <div className="dash-topbar-right">
        ...
        <button className="avatar" title={t.userFirst}
